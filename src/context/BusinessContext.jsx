...
export function BusinessProvider({ children }) {
    ...
    try {
      ...
      for (const doc of salesDocuments) {
        ...
        await supabase.from('sales_documents').upsert({
          ...
          receivable_posted: doc.receivable_posted == null
            ? doc.doc_type === 'sales_invoice' && Number(doc.balance_due) > 0
            : Boolean(doc.receivable_posted),
  ...
  const postSalesDocument = async (docData) => {
    ...
    const newDoc = {
      ...
      receivable_posted: docData.doc_type === 'sales_invoice' && balanceDue > 0,
    ...
    if (docData.doc_type === 'sales_invoice') {
      ...
      if (docData.customer_id || docData.customer_name) {
        ...
        const sourceReceivable = sourceDoc?.receivable_posted && String(sourceDoc.customer_id) === String(foundCust?.id)
          ? Number(sourceDoc.balance_due) || 0
          : 0;
        finalCustomerReceivable = Math.max(0, priorReceivable - sourceReceivable) + balanceDue;
    ...
    if (sourceDoc?.receivable_posted && isValidUUID(sourceDoc.customer_id) && Number(sourceDoc.balance_due) > 0) {
      await adjustCustomerBalance('Reversing reservation receivable', sourceDoc.customer_id, -Number(sourceDoc.balance_due), 0);
    }

    if (docData.doc_type === 'sales_invoice' && custId && balanceDue > 0) {
    ...
    if (sourceDoc && isValidUUID(sourceDoc.id)) {
      ...
      await runCloudWrite('Relinking reservation advances to invoice', () => supabase.from('payments').update({
        sales_doc_id: docId,
        reference: docNo
      }).eq('sales_doc_id', sourceDoc.id));
      await runCloudWrite('Relinking reservation cheques to invoice', () => supabase.from('cheque_register').update({
        sales_document_id: docId
      }).eq('sales_document_id', sourceDoc.id));
      setPayments(prev => prev.map(payment => String(payment.sales_doc_id || '') === String(sourceDoc.id)
        ? { ...payment, sales_doc_id: docId, reference: docNo }
        : payment));
      setCheques(prev => prev.map(cheque => String(cheque.sales_document_id || cheque.sales_doc_id || '') === String(sourceDoc.id)
        ? { ...cheque, sales_document_id: docId, sales_doc_id: docId, sales_doc_no: docNo }
        : cheque));
    }
  ...
  const updateSalesDocument = async (documentId, updatedData = {}) => {
    ...
    const oldReceivablePosted = Boolean(existingDoc.receivable_posted);
    const nextReceivablePosted = isInvoice && balanceDue > 0;
    ...
    await runCloudWrite('Updating sales document', () => supabase.from('sales_documents').update({
      ...
      receivable_posted: nextReceivablePosted,
    ...
    if (oldReceivablePosted && nextReceivablePosted && oldCustomerId === nextCustomerId) {
      if (nextCustomerId) await adjustCustomerBalance('Readjusting customer receivable', nextCustomerId, balanceDue - oldBalanceDue, 0);
    } else {
      if (oldReceivablePosted && oldCustomerId && oldBalanceDue) {
        await adjustCustomerBalance('Reversing previous customer receivable', oldCustomerId, -oldBalanceDue, 0);
      }
      if (nextReceivablePosted && nextCustomerId && balanceDue) {
        await adjustCustomerBalance('Recording new customer receivable', nextCustomerId, balanceDue, 0);
      }
    }
  ...
  const cancelReservation = async (docId) => {
    ...
    const linkedPayments = payments.filter(payment =>
      String(payment.sales_doc_id || '') === String(docId) ||
      payment.source_key?.startsWith(`sales:${docId}:`) ||
      (doc.doc_no && payment.payment_no?.includes(doc.doc_no))
    );
    const linkedPaymentIds = new Set(linkedPayments.map(payment => String(payment.id)));
    ...
    for (const payment of linkedPayments) {
      await reversePaymentBalance(payment, 'Reversing reservation advance');
    }
    if (doc.receivable_posted && isValidUUID(doc.customer_id) && Number(doc.balance_due) > 0) {
      await adjustCustomerBalance('Reversing reservation receivable', doc.customer_id, -Number(doc.balance_due), 0);
    }
    await runCloudWrite('Deleting reservation cheques', () => supabase.from('cheque_register').delete().eq('sales_document_id', docId));
    await runCloudWrite('Deleting reservation payments', () => supabase.from('payments').delete().eq('sales_doc_id', docId));

    setPayments(prev => prev.filter(payment =>
      String(payment.sales_doc_id || '') !== String(docId) &&
      !linkedPaymentIds.has(String(payment.id))
    ));
    setCheques(prev => prev.filter(cheque =>
      String(cheque.sales_document_id || cheque.sales_doc_id || '') !== String(docId) &&
      !linkedPaymentIds.has(String(cheque.payment_id))
    ));
    ...
    notifySuccess(`Reservation ${doc.doc_no} cancelled. Stock and any recorded advance were fully reversed.`);
  };
  ...
  const deleteSalesDocument = async (docId) => {
    ...
    if (doc.receivable_posted && doc.customer_id && Number(doc.balance_due) > 0) {
