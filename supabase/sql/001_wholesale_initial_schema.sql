...
CREATE TABLE IF NOT EXISTS supplier_advances (
    ...
    payment_method TEXT NOT NULL DEFAULT 'bank' CHECK (payment_method IN ('cash', 'bank', 'cheque', 'other')),
...
CREATE TABLE IF NOT EXISTS purchase_receipts (
    ...
    transit_shipment_id UUID NOT NULL UNIQUE REFERENCES transit_shipments(id) ON DELETE RESTRICT,
    supplier_id UUID NOT NULL REFERENCES suppliers(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS cheque_register (
    ...
    transit_shipment_id UUID REFERENCES transit_shipments(id) ON DELETE SET NULL,
    supplier_advance_id UUID REFERENCES supplier_advances(id) ON DELETE SET NULL,
    landed_cost_id UUID REFERENCES landed_costs(id) ON DELETE SET NULL,
    bank_name TEXT NOT NULL,
    branch TEXT,
...
CREATE TABLE IF NOT EXISTS payments (
    ...
    payment_type TEXT NOT NULL CHECK (payment_type IN (
        'customer_payment', 'customer_settlement', 'customer_advance', 'sales_receipt', 'customer_refund',
        'supplier_payment', 'supplier_advance', 'purchase_payment', 'transit_purchase_payment', 'supplier_refund',
        'expense', 'operational_expense', 'other_income', 'direct_income'
    )),
    party_type TEXT CHECK (party_type IN ('customer', 'supplier', 'other', 'payee', 'payer')),
    party_id UUID,
    payment_date DATE NOT NULL DEFAULT CURRENT_DATE,
    amount NUMERIC(12,2) NOT NULL CHECK (amount > 0),
    payment_method TEXT NOT NULL CHECK (payment_method IN ('cash', 'bank', 'card', 'cheque', 'customer_credit', 'credit', 'cod', 'other')),
    bank_account_id UUID REFERENCES bank_accounts(id) ON DELETE SET NULL,
    cheque_id UUID REFERENCES cheque_register(id) ON DELETE SET NULL,
    sales_doc_id UUID REFERENCES sales_documents(id) ON DELETE CASCADE,
    purchase_id UUID REFERENCES purchase_receipts(id) ON DELETE CASCADE,
    transit_shipment_id UUID REFERENCES transit_shipments(id) ON DELETE CASCADE,
    supplier_advance_id UUID REFERENCES supplier_advances(id) ON DELETE CASCADE,
    landed_cost_id UUID REFERENCES landed_costs(id) ON DELETE CASCADE,
