...
# 002_wholesale_functions_rpcs.sql
rpc_sql = """-- ==============================================================================
...
with open('supabase/sql/002_wholesale_functions_rpcs.sql', 'w', encoding='utf-8') as f:
    f.write(rpc_sql)

print('Created 002_wholesale_functions_rpcs.sql')

# 003_wholesale_seed_data.sql
seed_sql = """-- ==============================================================================
...
VALUES
    ('Commercial Bank - Wholesale Current', '1000123456', 'Commercial Bank of Ceylon', 'Pettah Branch', 'LKR', 500000.00, 500000.00),
    ('Sampath Bank - Corporate', '002910004567', 'Sampath Bank PLC', 'City Branch', 'LKR', 250000.00, 250000.00),
    ('HNB - Wholesale Operations', '034010998877', 'Hatton National Bank', 'Colombo Main', 'LKR', 150000.00, 150000.00)
ON CONFLICT DO NOTHING;

-- 6. Categories & Brands
INSERT INTO categories (name, code, sort_order)
VALUES
    ('Solid State Drives (SSD)', 'SSD', 1),
    ('Random Access Memory (RAM)', 'RAM', 2),
    ('Graphics Cards (GPU)', 'GPU', 3),
    ('Processors (CPU)', 'CPU', 4),
    ('Motherboards', 'MB', 5),
    ('Power Supply Units (PSU)', 'PSU', 6),
    ('Monitors & Displays', 'MON', 7),
    ('PC Cases & Cooling', 'CASE', 8),
    ('Networking & Cables', 'NET', 9),
    ('Computer Accessories', 'ACC', 10)
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO suppliers (
    supplier_code, name, country, contact_person, phone, email, address,
    default_currency, default_lead_days, bank_details, current_advance_balance, current_payable, products_supplied
) VALUES
    ('SUP-001', 'Shenzhen KingFast Storage Co., Ltd.', 'China', 'David Chen', '+86 755 8899 1122', 'david@kingfast-storage.cn', 'Bldg 4, High-Tech Industrial Park, Nanshan, Shenzhen', 'USD', 10, 'Bank of China Shenzhen / SWIFT: BKCHCNBJ400 / A/C: 456789012345', 0.00, 0.00, 'SATA SSD, NVMe SSD, DDR4/DDR5 RAM'),
    ('SUP-002', 'Guangzhou Redbox Electronics Ltd.', 'China', 'Mark Liu', '+86 20 8334 5566', 'mark@redbox-tech.com', 'Tianhe Computer City, Guangzhou, Guangdong', 'USD', 14, 'ICBC Guangzhou Branch / SWIFT: ICBCCNBSCAN / A/C: 887766554433', 0.00, 0.00, 'Motherboards, Graphics Cards, Power Supplies'),
    ('SUP-003', 'Hong Kong Global Tech Components', 'Hong Kong', 'Sarah Wong', '+852 2345 6789', 'sarah@hk-globaltech.hk', 'Unit 1204, Tower 2, Enterprise Square, Kowloon Bay, HK', 'USD', 7, 'HSBC Hong Kong / SWIFT: HSBCHKHHHKH / A/C: 112-233445-001', 0.00, 0.00, 'CPUs, OEM Memory, Flash Storage')
ON CONFLICT (supplier_code) DO NOTHING;

-- 8. Wholesale Customers
INSERT INTO customers (
    customer_code, business_name, contact_person, phone, whatsapp, email,
    billing_address, delivery_address, price_tier, credit_allowed, credit_limit, credit_days, current_receivable
) VALUES
    ('CUST-001', 'TechZone Solutions (Pvt) Ltd', 'Rizwan Mohamed', '+94 77 456 7890', '+94 77 456 7890', 'orders@techzone.lk', '220 Galle Road, Colombo 03', '220 Galle Road, Colombo 03', 'Dealer', TRUE, 1500000.00, 30, 0.00),
    ('CUST-002', 'Apex Computer World', 'Chaminda Perera', '+94 71 234 5678', '+94 71 234 5678', 'apexcomp@gmail.com', '15 Kandy Road, Kurunegala', '15 Kandy Road, Kurunegala', 'Tier1', TRUE, 750000.00, 14, 0.00),
    ('CUST-003', 'NextGen IT Systems', 'Kavinda Silva', '+94 76 890 1234', '+94 76 890 1234', 'info@nextgenit.lk', '88 Main Street, Kandy', '88 Main Street, Kandy', 'Standard', TRUE, 500000.00, 30, 0.00),
    ('CUST-004', 'Metro PC Hub', 'Sunil Jayawardena', '+94 77 998 8776', '+94 77 998 8776', 'metropchub@yahoo.com', '45 Station Road, Matara', '45 Station Road, Matara', 'Standard', TRUE, 300000.00, 14, 0.00)
ON CONFLICT (customer_code) DO NOTHING;
"""

with open('supabase/sql/003_wholesale_seed_data.sql', 'w', encoding='utf-8') as f:
    f.write(seed_sql)

print('Created 003_wholesale_seed_data.sql')
print('All SQL files built successfully!')
